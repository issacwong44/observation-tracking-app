...
const [handoverTab, setHandoverTab] = useState('observation')
...
async function handleAddPsyCase() {
  if (
    ...
    alert('Please complete Surname, Gender, Age, Chief Complaint and Location')
    return
  }
  ...
  const { error } = await supabase
    ...
    .insert([
      {
        observation_case_id: null,
        bed_no: null,

        patient_label: psySurname.trim().toUpperCase(),
...
>
  ...
  className="absolute top-3 right-3 bg-gray-100 hover:bg-blue-100 text-gray-500 hover:text-[#0078AE] px-2.5 py-1 rounded-xl text-[11px] md:text-xs font-bold transition"
>
  Edit
</button>
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Ambulatory psychiatric handover case
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {['M', 'F'].map((value) => (
              <button
                ...
                className={`py-4 rounded-2xl text-xl font-bold transition ${
                  psyGender === value
                    ? 'text-white'
                    : 'bg-gray-200 text-gray-700'
                }`}
        ...
        <div className="grid grid-cols-2 gap-3">
          {['Vol. form', 'F123'].map((value) => (
            <button
              key={value}
              type="button"
              onClick={() => setPsyAdmissionForm(value)}
